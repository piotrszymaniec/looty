...
- Added column mod "unparsed"
	...
	You can search them via matching text. For example:
	when want to find all "zombie" items you have, type "zombies" into "unparsed" column field and it should show all items,
	that have word "zombie" in affixes. I don't have any other solution for now. :)
- UI background readability improvements on Home, Xp, Wealth, Maps, Settings pages
  - switched off Maps and Poebuilder supages
...
With class PoeCacherJSON you have ability to load data from .json file when you place it in build directiory /data/ 
